                               ...
                               int *bestInd, int *bestDiff, int treeRoot)
{
    ...
    if (splitPos > splitComp)
    {
        ...
        GifGetClosestPaletteColor(pPal, r, g, b, bestInd, bestDiff,
                                  treeRoot * 2);
                          ...
                          int neededCenter)
{
    if (left < right - 1)
    {
        ...
        if (pivotIndex < neededCenter)
            GifPartitionByMedian(image, pivotIndex + 1, right, com,
                                 neededCenter);
    }
}
                         ...
                         int numPixels)
{
    ...
    for (int ii = 0; ii < numPixels; ++ii)
    {
        if (lastFrame[0] != frame[0] || lastFrame[1] != frame[1]
            || lastFrame[2] != frame[2])
        {
            writeIter[0] = frame[0];
            writeIter[1] = frame[1];
                    ...
                    bool buildForDither, GifPalette *pPal)
{
    ...
    if (lastFrame)
        numPixels =
            GifPickChangedPixels(lastFrame, destroyableImage, numPixels);

    const int lastElt = 1 << bitDepth;
    const int splitElt = lastElt / 2;
    const int splitDist = splitElt / 2;

    GifSplitPalette(destroyableImage, numPixels, 1, lastElt, splitElt,
                    splitDist, 1, buildForDither, pPal);
                    ...
                    GifPalette *pPal)
{
    ...
    // to be propagated
    int32_t *quantPixels =
        (int32_t *)GIF_TEMP_MALLOC(sizeof(int32_t) * (size_t)numPixels * 4);
    ...
    for (uint32_t yy = 0; yy < height; ++yy)
    {
        for (uint32_t xx = 0; xx < width; ++xx)
        {
            ...
            const uint8_t *lastPix =
                lastFrame ? lastFrame + 4 * (yy * width + xx) : NULL;
            ...
            if (lastFrame && lastPix[0] == rr && lastPix[1] == gg
                && lastPix[2] == bb)
            {
                nextPix[0] = rr;
                nextPix[1] = gg;
                       ...
                       GifPalette *pPal)
{
    ...
    for (uint32_t ii = 0; ii < numPixels; ++ii)
    {
        ...
        if (lastFrame && lastFrame[0] == nextFrame[0]
            && lastFrame[1] == nextFrame[1] && lastFrame[2] == nextFrame[2])
        {
        ...
        else
        {
            ...
            int32_t bestDiff = 1000000;
            int32_t bestInd = 1;
            GifGetClosestPaletteColor(pPal, nextFrame[0], nextFrame[1],
                                      nextFrame[2], &bestInd, &bestDiff, 1);
                      ...
                      GifPalette *pPal)
{
    ...
    for (uint32_t yy = 0; yy < height; ++yy)
    {
        for (uint32_t xx = 0; xx < width; ++xx)
        {
            ...
            // "loser mode" - no compression, every single code is followed
            // immediately by a clear
                   ...
                   bool dither = false)
{
    ...
    if (dither)
        ...
        GifThresholdImage(oldImage, image, writer->oldImage, width, height,
                          &pal);
