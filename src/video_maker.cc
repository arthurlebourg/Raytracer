...
void make_video_threads(Scene sc, int frames_begin, int frames_end, Color *res)
{
    for (int frame = frames_begin; frame < frames_end; frame++)
    {
        // Sample atmosphere
        // /*
        sc.objects_[1]->set_position(Vector3(-frame, 0, -frame / 2.0 + 300));
        sc.objects_[2]->set_position(Vector3(-frame, 0, -frame / 2.0 + 300));
...
void make_video(Scene &sc)
{
    ...
    {
        threads.push_back(std::thread(make_video_threads, sc.copy_for_thread(),
                                      i * frames_per_thread,
                                      (i + 1) * frames_per_thread, data));
    }

    make_video_threads(sc, (max_threads - 1) * frames_per_thread,
    ...
    std::string ffmpeg_sound = "ffmpeg -loglevel quiet -y -i raytracer.mp4 -i "
...
void make_image(Scene &sc)
{
    double y_num = img_height / max_threads;
    std::vector<std::thread> threads;
    Image img = Image("scene.ppm", img_width, img_height);
    for (int i = 0; i < max_threads - 1; i++)
    {
